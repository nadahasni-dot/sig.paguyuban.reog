...
class Auth extends CI_Controller
{
    ...
    public function index()
    {
        ...
        if (!$this->form_validation->run()) { // * jika form login belum diinput
            ...
            if ($data_user['status_user'] == 0) {
                $this->session->set_flashdata('message', '<div class="alert alert-info alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Akun anda belum diverifikasi, harap menunggu verifikasi dari pihak admin.</div>');
    ...
    // * function untuk halaman register
    public function registration()
    {
        ...
        $this->form_validation->set_rules('telepon_user', 'Phone', 'required|trim|numeric');
        $this->form_validation->set_rules('role', 'Role', 'required|trim');
        $this->form_validation->set_rules('email', 'Email', 'required|trim|valid_email');
        $this->form_validation->set_rules('password', 'Password', 'required|trim');
        $this->form_validation->set_rules('confpassword', 'Password Confirmation', 'required|trim|matches[password]');
        ...
        if (!$this->form_validation->run()) { // * jika form login belum diinput
            ...
            $this->load->view('template/auth/footer_auth_view');
        } else { // * jika sudah input form register
            $username = $this->input->post('username', true);
            $telepon_user = $this->input->post('telepon_user', true);
            ...
            if ($this->User_model->insertUser($data)) {
                $this->session->set_flashdata('message', '<div class="alert alert-success alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Berhasil melakukan registrasi. Harap tunggu verifikasi akun oleh admin untuk melanjutkan</div>');

                redirect('login');
            } else {
                $this->session->set_flashdata('message', '<div class="alert alert-danger alert-dismissible"><button type="button" class="close" data-dismiss="alert" aria-hidden="true">&times;</button>Gagal melakukan registrasi.</div>');
    ...
    public function forgotPassword()
    {
        ...
        if (!$this->form_validation->run()) { // * jika belum input form
            ...
            $data['title'] = 'Recovery Password Akun SIG PAGUYUBAN REOG';
            $data['heading'] = 'Recovery Password Akun SIG PAGUYUBAN REOG';
